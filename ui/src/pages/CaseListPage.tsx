...
export const CaseListPage: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-5 font-mono">
      {/* ── Top Header ──────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 sm:pb-4 border-b border-slate-200 gap-2 sm:gap-3">
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 sm:gap-3">
        <div className="sm:col-span-6 relative">
          ...
          <input
            type="text"
            placeholder="Search by case ID, customer, card..."
            value={searchTerm}
        ...
        <div className="sm:col-span-3">
          <select
            ...
            className="w-full bg-white border border-slate-200 rounded-lg text-xs text-slate-700 px-3 py-2 focus:outline-none focus:border-blue-500 shadow-sm"
          >
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Link to={`/cases/${c.case_id}`} className="font-bold text-blue-600 text-sm hover:underline">
                    {c.case_id}
                  </Link>
                  <span
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                <div>
                  ...
                  <span className="text-slate-400 ml-1">· Txn #{c.flagged_txn_id}</span>
                </div>

                <div className="flex items-center gap-2">
                  <span>Model: <strong>{(bScore * 100).toFixed(0)}</strong></span>
                  <span>·</span>
                  <span className={aProb > 0.7 ? "text-rose-600 font-bold" : "text-emerald-600 font-bold"}>
                    Prob: {(aProb * 100).toFixed(0)}%
                  </span>
                </div>
              </div>
            </div>
      ...
      {/* ── Desktop Crisp Light Table (>= md) ──────────────────────────────── */}
      <div className="hidden md:block rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
